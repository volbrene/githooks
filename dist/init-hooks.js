...
function sh(cmd, opts = {}) {
    return (0, child_process_1.execSync)(cmd, { stdio: ['ignore', 'pipe', 'pipe'], ...opts })
        .toString()
        .trim();
}
...
let hooksDir = safeSh('git rev-parse --git-path hooks', { cwd: repoRoot }) ||
    path.join(repoRoot, '.git', 'hooks');
console.log(`📂 Using hooks directory: ${hooksDir}`);
ensureDirSync(hooksDir);
async function addGitHook(hookName, sourceDir, targetDir) {
    ...
    try {
        ...
        console.error(`❌ Failed to install hook "${hookName}": ${(err === null || err === void 0 ? void 0 : err.message) || err}`);
    }
}
(async () => {
    ...
    console.error(`❌ Unhandled error during hooks setup: ${(err === null || err === void 0 ? void 0 : err.message) || err}`);
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    process.exit(1);
});
